 ...
 */
export default function KpiCard({ value, label, sublabel, info, className = '', style = {}, animate, fillPct, fillColor, onClick }) {
  ...
  return (
    <div className={`card kpi-card ${className}`} style={{ ...(style?.color ? { '--kpi-accent': style.color } : {}), ...(onClick ? { cursor: 'pointer' } : {}) }} onClick={onClick}>
      {info && <div className="kpi-info-corner"><InfoTip text={info} /></div>}
      {fillPct > 0 && (
        <div
